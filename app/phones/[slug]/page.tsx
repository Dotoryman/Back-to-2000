...
export default async function PhonePage({params}:{params:Promise<{slug:string}>}){const slug=(await params).slug;const [item,catalog]=await Promise.all([getPublishedCatalogItemBySlug("phone",slug),listPublishedCatalog()]);if(!item)notFound();return <CatalogDetail item={item} next={findNextEvolution(item,catalog)} lineage={findEvolutionLineage(item,catalog)}/>}
