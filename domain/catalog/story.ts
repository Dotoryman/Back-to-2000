...
export function buildCatalogStory(item: CatalogItem, next?: CatalogItem): CatalogStory {
  ...
  return {
    ...
    significance: item.story?.significance ?? `${item.name}의 의미는 기능의 수보다 그 시대 사람들이 디지털 기술을 대하는 방식을 바꿨다는 데 있습니다. ${features}은(는) ${item.year}년 무렵의 기대와 취향을 선명하게 보여주며, 비슷한 제품과 서비스가 따라갈 하나의 기준을 만들었습니다.`,
