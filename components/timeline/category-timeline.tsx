...
export function CategoryTimeline({ kind, catalog }: { kind: TimelineKind; catalog: CatalogItem[] }) {
  ...
  return <section className={`category-timeline category-${kind}`}>
    <aside className="category-year-rail"><span>{kind === "website" ? "WEB" : kind === "phone" ? "MOBILE" : "GLOBAL GAME"} HISTORY</span><strong>{year}</strong><input type="range" min="0" max={timelineYears.length - 1} value={index} onChange={(event) => selectIndex(Number(event.target.value))} aria-label={`${kind === "website" ? "웹사이트" : kind === "phone" ? "휴대전화" : "게임"} 연도 선택`} /><div>{timelineYears.map((item) => <button key={item} className={item === year ? "active" : ""} onClick={() => setYear(item)}>{item}</button>)}</div></aside>
